...
void Aircraft::PhysicsLoop() 
{
  ...
  Vector3
      liftDir; // for now assume that aircraft does not lean to right and left
  ...
  while (isRunning) 
  {
    ...
    this_thread::sleep_for(microseconds(
        500)); // lets say physics calculations took half of a milliseconds

    // force calculations
    gravityForce = gravityVector * mass; // calculate the gravitational force

    liftForceMag = 0.5f * RHO * velocity.LengthSquared() * wingArea *
                   liftCoefficient; // v^2'den emin olamadım vektörün karesini
                                    // mi almam lazımdı böyle?
    liftDir = orientation.RotateVector(Vector3(0, 1, 0));
    liftForce = liftDir * liftForceMag;

    dragForceMag =
        0.5f * RHO * velocity.LengthSquared() * wingArea * dragCoefficient;

    dragDir = (velocity * -1.0f).Normalized();
    dragForce = dragDir * dragForceMag;

    thrustMag = GetCurrentThrust();
    thrustDir = orientation.RotateVector(
        Vector3(0, 0, 1)); // lets assume that aircraft looks at (0,0,1)
                           // locally. now we find the thrust direction in real
                           // world by multiplying it with its orientation
    ...
    // if the aircraft is fast enough bend the velocity vector to the aircraft
    // look direction
    ...
    { // to be able to release the lock immediately
      ...
      // damping factor decreases as velocity increases and rotating is
      // suppressed
      float dampingFactor = 1.0f - (0.01f + (velocity.Length() * 0.0001f));
      if (dampingFactor < 0.5f)
        dampingFactor = 0.5f; // bound it

      angularVelocity = angularVelocity * dampingFactor; // aerodynamic damping

      float angleRad = angularVelocity.Length() *
                       dt; // how many radians we turned in dt time?

      if (angularVelocity.LengthSquared() >
          0.000001f) // to avoid divide by zero
      {
        // update the orientation with that angularVelocity
        // find the amount of rotation
        Quaternion deltaRot = Quaternion::AngleAxis(
            angularVelocity.Normalized(), angleRad * (180.0f / PI));

        // multiply the old orientation with the new rotation amount to find new
        // orientation
    ...
    this_thread::sleep_until(
        target_time); // job is done sleep until the target time
  ...
  while (isRunning) {
...

    {
      ...
      case FlightState::CRUISE:
        ...
        {
          aileronDeflection = 0.0f; // we assume in 3 seconds aircrraft became
                                    // straight by completing rolling
        } else {
          aileronDeflection = -1.0f;
        }
      ...
      case FlightState::FAIL_SAFE:
        currentThrust =
            0; // bu kısım daha akıllı olmalı. hata tipine bağlı olarak başta
               // başladığı yere dönme olabilir bunun için uçağın başlangıç
               // konumunu tutmalıyız ve hareket ettikçe güncellemeliyiz
               // konumunu, fail safe olunca da geri dönebilir.
        break;
      }
    }

    this_thread::sleep_for(
        milliseconds(5)); // for now assume control job takes 5 milliseconds
...
void Aircraft::TelemetryLoop() 
{
  ...
  while (isRunning) 
  {
    ...
    this_thread::sleep_for(
        milliseconds(50)); // for now assume telemetry job takes 50 milliseconds

    {
      lock_guard<mutex> lock(stateMutex);

      logFile << (i * 0.1f) << "," << position.y << "," << velocity.y << ","
              << currentThrust << "," << orientation.x << "," << orientation.y
              << "," << orientation.z << "," << orientation.w << "\n";

      char buffer[256];
      // we write the time altitude and 4 orientation data into a c-like string
      // that is gonna be sent to python script
